...
\seminormal
\begin{itemize}
	\item \appear{Recently}\appear{, \CDAlert[red]{high-energy physics}} \appear{and \CDAlert[blue]{cosmology} have approached each other}
	
	\item This is because the theories describing \CDAlert[fuchsia]{fundamental interactions} \appear{are linked to the early history of the universe}\appear{, right after the \CDAlert[fuchsia]{Big Bang}}\appear{, and to the origin of the fundamental particles and matter in the universe}

	\item For some time after the Big Bang \appear{the energies were quite high}\appear{, exceeding the atomic bonding energies}\appear{, matter consisted of \CDAlert[red]{electron and positive ion plasma}} \appear{\CDAlert[tunired]{(opaque to radiation)}}
	
	\item Gradually the temperature dropped, matter turned transparent, enabling propagation of \CDAlert[blue]{cosmic background radiation} 
	\item Presently the \Alert[tuniblue]{cosmic background radiation} is of the order of $2.7 \mathrm{\;K}$\appear{, which value is in agreement with the present theories on the \CDAlert[blue]{universe} \CDAlert[blue]{expanding after Big Bang}}
	\implies{ How radiation from an originally hot object has cooled down to ${\Approx}\;2.7$\;K?  }
\end{itemize}
...
\begin{itemize}
	\item \appear{Quite \Alert[fuchsia]{analogous to the escaping projectile}}\appear{, we can assume that \CDAlert[fuchsia]{universe} is an expanding sphere}\appear{, with size $R=R(t)$}\appear{, actually called a \CDAlert[red]{scale factor}} 
	
	\item Expansion is described by \Alert[blue]{Friedmann equation}:
\[
...
\item If \Alert[green]{$K=0$}: \appear{a flat universe}\appear{, expansion constant to infinite time $t \oldrightarrow \infty$ }
...
\begin{itemize}
	\item \appear{Next, we modify the density in the equation:} $\appear{\rho} \rightarrow \appear{\rho_{M} /{R^{3}}}  \plus \appear{\rho_{\Lambda}}$ \appear{where $\rho_{M} /R^{3}$ is the present density of the universe} \appear{and $\rho_{\Lambda}$ is a factor connected to the \CDAlert[fuchsia]{cosmological constant}} \appear{(introduced later):}
\[
 \textrm{((} \frac{d R / d t}{R} \appear{\textrm{))}^2}   \equal \frac{8 \pi G \rho_{M}}{3 R^{3}}  \plus \frac{8 \pi G \rho_{\Lambda}}{3}  \minus \frac{K}{R^{2}}
\]
\appear{Finally, we rearrange the \CDAlert[blue]{Friedmann equation} to its final form:}
\[
...
\begin{itemize}
	\item \appear{On the tuning parameters:} %\vspace{1mm}
	\begin{itemize}
...
\begin{itemize}
	\item[] $$
	\begin{aligned}
	\appear{\textrm{((}} \frac{d R / d t}{R} \appear{\textrm{))}^2} & \equal  \frac{1}{R^{3}} \quad \Rightarrow \quad \appear{\nint_{1}^{R}} \appear{R^{1 / 2}} \appear{\rmd R}  \equal \appear{\nint_{1}^{t} d t} \\
\Rightarrow \quad \appear{R(t)} & \equal  \appear{\textrm{[[} \Frac{3}{2}} \appear{( t-\Frac{1}{3} )} \appear{\textrm{]]}^{2 / 3}}
	\end{aligned}
$$
\item[] $\Omega_{\Lambda}=$ \CDAlert[red]{cosmological constant}\appear{, potentially determining whether the \CDAlert[red]{expansion} of the universe will be \Alert[red]{constant or not}} \appear{(see Fig.~17)}
\end{itemize}
\end{minipage}
%\vspace{2.5mm}
...
\begin{itemize}
	\item \appear{Earlier, we looked into proton–proton cycle occurring within stars}

\item In fact, history of the whole universe has some similarities

\item At the present knowledge \appear{((called the standard model of the history of the universe))}\appear{, the temperature of the universe at a time  $t\Approx10^{-46} \mathrm{\;s}$ was about $10^{32} \mathrm{\;K}$}\appear{, and the average energy per particle was:} %\vspace{-4mm}
\[
...
\item At $t=10^{-36} \mathrm{\;s}$ the temperature had decreased to  ${\sim}10^{27} \mathrm{\;K}$\appear{, the energy being ${\sim}10^{14} \mathrm{\;GeV}$.} \appear{\Alert[fuchsia]{At this energy the strong force separated from electroweak force}} \appear{\CDAlert[black!20!fuchsia]{(end of grand unification)}}
\end{itemize}
...
\begin{itemize}[itemsep=1.7mm]
	\item \appear{At $\mathrm{t}=10^{-32} \mathrm{\;s}$} \appear{the temperature had decreased to ${\sim}10^{25} \mathrm{\;K}$} \appear{and the universe was a mixture of \CDAlert[fuchsia]{quarks}}\appear{, \CDAlert[blue]{leptons}} \appear{and the \CDAlert[red]{mediating bosons}}
	
	\item At $\mathrm{t}=10^{-6} \mathrm{\;s}$ \appear{the temperature had decreased to about $10^{13} \mathrm{\;K}$} \appear{and the typical energy was about $1 \mathrm{\;GeV}$} \appear{(comparable to rest mass of a nucleon).} \appear{\CDAlert[fuchsia]{Quarks began} \CDAlert[fuchsia]{binding together to from nucleons}}
...
\begin{frame}
\frametitle{\texorpdfstring{\culine[\sectiontitleulinecolor]{\sectiontitle}}{\sectiontitle} }
\framesubtitle{History/evolution of the universe (a la Harris)}

\xdef\loc{south}
\begin{tikzpicture}[remember picture,overlay] % anchor=south
    \node (A) [yshift=-0.25*\figYshift,xshift=\figXshift, anchor=\loc] at (current page.\loc) {\includegraphics[page=1,height=8.8cm]{Figures_booklet/SOM_11_Fundamental_particles_figures/SOM_Fundamental_particles_Figure_18.png}}; %scale=\figscale. % 8cm max hei
\end{tikzpicture}
...
\begin{frame}
\frametitle{\texorpdfstring{\culine[\sectiontitleulinecolor]{\sectiontitle}}{\sectiontitle} }
\framesubtitle{History/evolution of the universe (a la Tipler)}
...
\begin{frame}
\frametitle{\texorpdfstring{\culine[\sectiontitleulinecolor]{\sectiontitle}}{\sectiontitle} }
\framesubtitle{But BIG open questions remain..}

\vspace{2.5mm}
\begin{minipage}{5.0cm}
\begin{itemize}
	\item \appear{Only 4 \% of mass is \Alert[fuchsia]{baryonic}?} \appear{(non-baryonic:} \appear{neutrinos}\appear{, free electrons}\appear{, black holes}\appear{, dark matter}\appear{, supersymmetric particles}\appear{,\,...)}
	
	\item Galaxy rotation curves \appear{imply existence of \CDAlert[blue]{dark matter?}}
	\item Expansion of universe is \CDAlert[red]{accelerating} \appear{because of \CDAlert[red]{dark energy?}} 
	\item Quantum field theory for gravity\,...
\end{itemize}
\end{minipage}
%\vspace{2.5mm}
...
%\begin{tikzpicture}[remember picture,overlay] % anchor=south
%    \node (A) [yshift=-2cm, xshift=3cm, anchor=west] at (current page.north) {Galary rotation curves and dark matter}; %scale=\figscale. % 8cm max hei
%\end{tikzpicture}
